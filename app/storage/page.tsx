...
import { HardDrive, FileAudio, MoreVertical, User, CreditCard, LogOut, Image as ImageIcon, Trash2, AlertCircle, CheckCircle2, Loader2, RefreshCw, Eye, Play, X, Music2 } from "lucide-react";
...
export default function StoragePage() {
  const { playlist, userPlan } = useAudio();
  ...
  // Calculer l'utilisation et le statut (Utilisé / Orphelin)
  const processedFiles = files.map(file => {
      let associatedTrack: any = null;
      let versionLabel = "";

      // Nettoyage du nom de fichier (retirer le timestamp ajouté par R2)
      const nameWithoutTimestamp = file.name.replace(/^\d+-/, '');
      // On retire aussi l'extension pour un affichage plus propre
      const cleanName = nameWithoutTimestamp.replace(/\.[^/.]+$/, "");

      // Vérifier si ce fichier est référencé dans la playlist (Tracks, Versions, Covers)
      const isUsed = playlist.some((track) => {
          // Comparaison basée sur le chemin (fullPath) et non l'URL complète (plus fiable)
          const mainPath = track.url ? getStoragePathFromUrl(track.url) : null;
          const coverPath = track.coverUrl ? getStoragePathFromUrl(track.coverUrl) : null;
          
          const isMain = mainPath === file.fullPath;
          const isCover = coverPath === file.fullPath;
          
          const matchingVersion = track.versions?.find(v => {
              const vPath = v.url ? getStoragePathFromUrl(v.url) : null;
              return vPath === file.fullPath;
          });

          if (isMain || isCover || matchingVersion) {
              associatedTrack = track;
              if (matchingVersion) {
                  versionLabel = ` (${matchingVersion.name})`;
              }
              return true;
          }
          return false;
      ...
      const isOrphan = !isUsed && file.category === 'image';
      ...
      return {
          ...
          isUsed,
          isOrphan,
          associatedTitle: associatedTrack ? `${associatedTrack.title}${versionLabel}` : (isOrphan ? "Image inutilisée" : cleanName),
          associatedArtist: associatedTrack ? associatedTrack.artist : ""
      };
  });
  ...
  return (
    <div className="h-full w-full overflow-y-auto bg-black pb-40">
                        ...
                        <tr key={file.id} className="hover:bg-[#1a1a1a] transition-colors">
                            ...
                            <td className="px-6 py-4 text-right">
                                <div className="flex items-center justify-end gap-2">
                                    ...
                                    <button onClick={() => handleDeleteFile(file)} className="p-2 text-zinc-500 hover:text-red-400 hover:bg-red-400/10 rounded-full transition-colors" title={file.isUsed ? "Fichier utilisé (impossible de supprimer)" : "Supprimer ce fichier"}>
                                        <Trash2 size={16} />
                                    </button>
                                </div>
                            </td>
